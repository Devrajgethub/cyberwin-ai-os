...
import { Sun, Moon, Wifi, Battery, Volume2, Search, LayoutGrid } from 'lucide-react';
    ...
    <div className="fixed bottom-0 left-0 right-0 h-12 z-50 no-select">
      ...
      <div className="relative z-10 h-full flex items-center px-2 gap-1">
        ...
        <div className="flex items-center gap-1">
          <button
            className={`flex items-center justify-center w-10 h-10 rounded-lg
              ...
              ${isStartMenuOpen
                ? 'bg-cyan-500/20 shadow-[0_0_12px_rgba(6,182,212,0.2)]'
                : 'hover:bg-white/10'
              }`}
            onClick={(e) => {
              e.stopPropagation();
          ...
          {taskbarApps.map((app) => (
            <button
              ...
              className={`flex items-center gap-2 h-10 px-3 rounded-lg
                transition-all duration-150 max-w-[180px] min-w-[44px]
                ${app.isActive && !app.isMinimized
                  ? 'bg-white/[0.08] border-b-2 border-cyan-400'
                  : app.isMinimized
                    ? 'hover:bg-white/[0.05] opacity-60'
                    : 'hover:bg-white/[0.05]'
                }`}
              onClick={(e) => {
                e.stopPropagation();
                handleAppClick(app.windowId, app.isMinimized);
              }}
              title={app.title}
            >
        ...
        <div className="flex items-center gap-0.5">
          <button
            className="flex items-center justify-center w-8 h-10 rounded-lg hover:bg-white/10 transition-colors"
            onClick={toggleTheme}
          >
            {theme === 'dark' ? (
          ...
          <div className="flex flex-col items-end justify-center h-10 px-2 text-gray-300">
            <span className="text-[11px] leading-tight font-medium">{formatTime(time)}</span>
            <span className="text-[10px] leading-tight text-gray-500 hidden sm:block">{formatDate(time)}</span>
          </div>
        </div>
      </div>
    </div>
